# Adopt Collocated Unit Tests with .test Extension for Frontend Components: Test Files Use
...
- **R-TEST-001** MUST: Test files MUST use the exact base name of the source file followed by .test.tsx or .test.ts (e.g., password-input.tsx → password-input.test.tsx)
...
# Verify test runner configuration includes proper test discovery patterns
...
<enforcement>
Claude Code MUST NOT skip or defer verification of collocated test file presence and naming conventions. CI/CD pipeline MUST fail if new source files are committed without corresponding test files. Pre-commit hooks MUST check for presence of test files alongside new source files.
</enforcement>