# Adopt React Hook Form with Zod for Client-Side Form Validation and State Management: Form Components Define

These rules are ALWAYS ACTIVE for all user-facing form components including authentication flows, settings and configuration forms, and data entry forms that accept user input requiring validation and submission.

### Rules

- **R-FORM-001** MUST: Form components MUST define TypeScript types derived from Zod schemas using `z.infer<typeof schema>`
...
<enforcement>
Clause Code MUST NOT skip or defer verification of R-FORM-001 compliance. All new form components and modifications to existing forms must be verified against these rules before approval.
</enforcement>